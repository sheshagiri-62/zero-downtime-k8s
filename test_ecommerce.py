...
def test_endpoints(url, name):
    ...
    print("[PASS] /api/products returns products")
    ...
    orders = r.json()
    assert len(orders) > 0
    print(f"[PASS] Retrieved orders. App version recorded: {orders[0]['app_version']}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            assert r.status_code == 200
            print("[PASS] Stable /health works")
